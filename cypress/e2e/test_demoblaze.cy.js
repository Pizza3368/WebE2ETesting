describe('User Registration, Login, Product Search, Checkout, Navigation, and Contact Form Tests', () => {
  ...
  it('Should show error for invalid checkout details (Negative Test)', () => {
    ...
    // Ensure that the success modal does NOT appear
  ...
  it('Should successfully checkout with valid details (Positive Test)', () => {
    ...
    // Check for the success modal content specifically in the SweetAlert modal
  ...
  it('Should successfully send a message with filled contact form (Positive Test)', () => {
    // Click the Contact link to open the modal
    cy.get('a.nav-link').contains('Contact').then(($contactLink) => {
      if ($contactLink.is(':visible')) {
        cy.wrap($contactLink).click({ force: true });
      }
    });
    cy.wait(500);
    cy.get('#exampleModal').invoke('show');
